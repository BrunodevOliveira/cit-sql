...
export function activate(context: vscode.ExtensionContext) {
    let disposable = vscode.commands.registerCommand('extension.executeSqlQuery', async () => {
        ...
        const { endpoint, token, query } = extractEndpointTokenAndQuery(text);
        ...
        try {
            ...
            await vscode.workspace.fs.writeFile(uri, Buffer.from(createTableView(data, finalEndpoint)));
...
function extractEndpointTokenAndQuery(text: string): { endpoint?: string; token?: string; query: string } {
...
function createTableView(data: any[], endpoint: string): string {
